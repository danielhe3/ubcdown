...
## Quality control of identified datasets
...
Lists of differentially expressed genes between ILD and healthy control samples were extracted from each study and ordered according to fold-change (decreasing for up-regulated, increasing for down-regulated). DEG lists for fibrotic ILD (IPF, HP, SSc-ILD) against controls were extracted from each study, along with fold change, p-value, and false discovery rate (FDR). Fold change-ordered lists were analyzed using the RRA package (v1.2.1) to generate a consensus list of DEGs, which was Bonferroni-corrected as described [@vosa_comprehensive_2014]. A fourth aggregated list was generated for all ILDs using the aforementioned subtype-specific lists in addition to three lists published by Zuo, Yang, and Horimasu comparing ILD lung against control lung samples [@zuo_gene_2002, @yang_gene_2007, @horimasu_gene_2017]. 
...
```{r outlierPCA, eval = T, echo = T, out.width='1\\linewidth',echo=FALSE, fig.cap="(ref:outlier-cap)", fig.scap = "Outlier detection", out.extra=''}
knitr::include_graphics("./Figures/FigE1_outliers.pdf")
```
...
```{r integration, eval = T, echo = T, out.width='1\\linewidth',echo=FALSE, fig.cap="(ref:int-cap)", fig.scap = "MINT integration", out.extra=''}
knitr::include_graphics("./Figures/Figure2_PCA.pdf")
```
...
```{r tuningMINT, eval = T, out.width='1\\linewidth', echo=FALSE, fig.cap="(ref:tuning-cap)", fig.scap = "Model tuning", out.extra=''}
knitr::include_graphics("./Figures/FigE2_tuning.pdf")
```
...
kableExtra::kbl(
  readr::read_csv(here::here('Tables', 'MINT_performance.csv')),
  longtable = FALSE,
  format = 'latex',
  booktabs = TRUE, 
  linesep = "",
...
kableExtra::kbl(
  readr::read_csv(here::here('Tables', 'Table E5 Sex Model Performance.csv')),
  longtable = FALSE,
  format = 'latex',
  booktabs = TRUE, 
  linesep = "",
...
```{r nsipmodel, eval = T, out.width='1\\linewidth', echo=FALSE, fig.cap="(ref:nsipmodel-cap)", fig.scap = "Model tuning", out.extra=''}
knitr::include_graphics("./Figures/FigE5_NSIP.pdf")
```
...
```{r ildvsildspecific, eval = T, out.width='1\\linewidth', echo=FALSE, fig.cap="(ref:ildvsildspecific-cap)", fig.scap = "Individual ILD vs ILD", out.extra=''}
knitr::include_graphics("./Figures/FigE6_ILDvsILD.pdf")
```
...
Gene lists from expanded models were then analyzed via overrepresentation analysis using the ‘cashoes/sear’ package (https://github.com/cashoes/sear) (Table E8). 
...
```{r biclusterGenes, eval = F, echo = F, warning = FALSE, message=FALSE,  out.extra=''}
library(readxl)    
...
kable(biclusters,
      ...
      longtable = TRUE,
      caption = "example") %>% 
  kableExtra::kable_styling(full_width = TRUE, font_size = 6, latex_options = c("hold_position", "scale_down", "repeat_header"))
```
...
library(dplyr)
table <- readr::read_csv(here::here('Tables', 'SysReviewQC.csv'))
